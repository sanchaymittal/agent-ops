...
- **Wait wide, not often.** Wake only for completion, escalation, terminal exit, or a user-visible checkpoint — never a `sleep`/poll loop. One blocking wait covering every outstanding worker, window ≥ 15 minutes — real coding tasks run 15–60 min, so a shorter window buys round-trips and nothing else. Take exact flags from the substrate's own guide, never from this file. Never a 30s yield, never one wait per worker, never a poll loop between waits. A timeout is a checkpoint, not a failure: re-wait. Each poll re-sends the coordinator's whole context to return a few KB — measured on a real run, polling was 47% of all tokens burned while `apply_patch` was 2%.
- **Polling budget: ≤ 2 supervision calls per dispatched task.** Past that, the workers are slower than assumed — block on the next `worker_done` with a longer timeout, or tell the user, instead of watching.
