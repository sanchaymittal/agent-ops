...
- **Wait wide, not often.** Wake only for completion, escalation, terminal exit, or a user-visible checkpoint — never a `sleep`/poll loop. One blocking wait covering every outstanding worker, window ≥ 15 minutes — real coding tasks run 15–60 min, so a shorter window buys round-trips and nothing else. Take exact flags from the substrate's own guide, never from this file. Never a 30s yield, never one wait per worker, never a poll loop between waits. A timeout is a checkpoint, not a failure: re-wait. Each poll re-sends the coordinator's whole context to return a few KB — measured on a real run, polling was 47% of all tokens burned; the calls that applied patches were 2%, and that 2% is a floor, since a patch applied through a shell counts as work.
...
- **Bound the re-dispatch loop.** "Until it passes" is not a stop condition. Fix the number of review → fix rounds before the first dispatch — three unless the task states otherwise — and write that cap into the prompt file. At the cap, stop and report what still fails instead of dispatching again. An unbounded loop re-sends the coordinator's whole context on every round, so its cost grows with the number of rejections, and nothing in it stops before the quota funding it runs out. A user asking to loop until approval is asking for the outcome, not for an uncapped loop: state the cap back to them.
