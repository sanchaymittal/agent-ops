...
The coordinator runs the loop (executor); the PLANNER slot ([models.md](./models.md)) is the on-demand advisor. Consult at defined triggers only.
...
**Consult mechanic**: coordinator makes a one-shot, non-interactive call to the PLANNER model with the specific question plus minimal context — relevant file excerpts, exact error output, the two options if a tie-break. Never a full transcript or bulk file dump. Advice returns as text; coordinator appends it to the task's prompt file under a `## Advice` section, then respawns the same prompt file. The prompt file stays the idempotent dispatch unit — respawned workers carry the advice.
